...
{
by Velislei Adilson Treuk - 31-AGO-2011

ERROS:

Versão com Erros
A variavel resposta parece pegar somente o primeiro registro
...
Scanear()
Huawei5103()
Script( telnet TEBAJ661,  Huawei MA5103 Multi-service Access Module. Copyright(C) 1998-2002 by Huawei Technologies Co., Ltd.> User name (<20 chars): )
Script( root, > Password (<20 chars): )
Script( admin, TEBAJ661> )
Script( enable, TEBAJ661# )
Script( conf ter, TEBAJ661(config)# )
Script( int adsl 0/4, TEBAJ661(config-ADSL-0/4)# )
Script( show line oper 0, TEBAJ661(config-ADSL-0/4)# )
Profile5103( 15, 320, 1024)
ShState( Sincronizada )
EditarMySql( TEBAJ661-04/00 )
***
	Consulta Portas (Script...)
***
Script( exit, TEBAJ661(config)# )
Script( end, TEBAJ661# )
Script( exit, TEBAJ661> )
...
type
  ...
  private
    ...
    function NecB820(ID_Dslam: String; num_placas: Integer): boolean;
    function NecB1000(ID_Dslam: String; num_placas: Integer): boolean;
    ...
    procedure ProfileB820(var Profile, Up, Dn: String);
    procedure ProfileB1000(var Profile, Up, Dn: String);
...
begin
        ...
        Inicializar();                                                          // Seta valores iniciais
...
begin
        ...
        Teste_num_placas := 2;
        Teste_num_portas := 2;                                                  // Limita num de portas por placas
        resposta_num := 0;
...
MemoShell.Lines.Add('Iniciando conexão! Aguarde...');
...
//    MemoLogErro.Lines.Add('ssh.SessionLog);
    ...
    Exit;
        ...
        prompt_anterior := 'bash-2.03$';

        result := resultado;

end; // final função Conectar
...
// Inicia varredura
       ...
       if MenuRastrear2.Checked then Rastrear('Scanear()');
        ...
        ZTbDslamNec.last;                             // Aponta para o ultimo registro
        Total_Dslam := StrToInt(DbEdDslam_Reg.Text);    // Pega total de Dslam´s na tabela
        for TD := 1 to Total_Dslam do
        begin
             ZTbDslamNec.Locate('registro',TD,[]);          // Aponta para registro correspondente
...
//             if (Versao_Dslam = 'B820') then MemoComandos.Lines.Add('Id.cadastro ! Conexão com: ' + IntToStr(TD) + '=' + Nome_Dslam + '-' + Versao_Dslam + '-' + IntToStr(Num_Placas) );
             if (Versao_Dslam = 'B820') then NecB820(Nome_Dslam,Num_Placas);  // Executa acesso ao Huawei 5100

             if (Versao_Dslam = 'B1000') then MemoComandos.Lines.Add('Id.cadastro ! Conexão com: ' + IntToStr(TD) + '=' + Nome_Dslam + '-' + Versao_Dslam + '-' + IntToStr(Num_Placas) ); // Huawei5103(Nome_Dslam,Num_Placas);  // Executa acesso ao Huawei 5103
//             if (Versao_Dslam = 'B1000') then NecB1000(Nome_Dslam,Num_Placas);  // Executa acesso ao Huawei 5600
...
function TMain.NecB820(ID_Dslam: String; num_placas: Integer): boolean;
var
...
begin
	 if MenuRastrear2.Checked then Rastrear('NecB820()');
	 
    //******************************************************************************
// Script´s
        Script('telnet ' + ID_Dslam,'2.4 ICM3 Board10.20.30.1 login:');   // Esta dando erro mas entra - verificar
        Script('admin','Password:');
        Script('N3cp@s',ID_Dslam + '#');

       //Entra no slot
        for slot_adsl:=1 to teste_num_placas do
        begin
          cmd_slot := 'slot ' + IntToStr(slot_adsl);                              // Comando
          prompt_slot := ID_Dslam + '(IPADSL3A-' + IntToStr(slot_adsl) + ')#';      // Retorno
          Script(cmd_slot,prompt_slot);                                                 // Script a enviar

          cmd_slot := 'port';                              // Comando
          prompt_slot := ID_Dslam + '(IPADSL3A-' + IntToStr(slot_adsl) + '/PORT)#';      // Retorno
          Script(cmd_slot,prompt_slot);                                                 // Script a enviar
          ...
          begin
                ...
                cmd_porta := 'show line dsl ' + IntToStr(porta_adsl);                 // Comando a enviar
                prompt_porta := ID_Dslam + '(IPADSL3A-' + IntToStr(slot_adsl) + '/PORT)#';
                Script(cmd_porta,prompt_porta);

                if ShState('B820') = 'Sincronizada' then begin
                        cmd_porta := 'Q';                 // Comando a enviar
                        prompt_porta := ' Press any key to continue (Q to quit)';
                        Script(cmd_porta,prompt_porta);
                end;

                ProfileB820(Profile,Up,Dn);                         // Analisa Vel Up-Down
                EditarMySql(ID_Porta,ShState('B820'),Profile,Up,Dn);      // Atualizar registros no BD MySql

                // Mostrar relatorio da consulta
                // State(): verifica se Inativa, Ativa ou bloqueada
                // Profile(): Analiza a velocidade da Porta
                MemoResultado.Lines.Add('[' + ID_Porta + ']' + ' - ' + ShState('B820') + ' - ' + Up + '-' + Dn );

          end;  // for porta_adsl
           //********************************************************************************
          Script('home',ID_Dslam + '#');
         end; // for slot_adsl
...
function TMain.NecB1000(ID_Dslam: String; num_placas: Integer): boolean;
var
   slot_adsl, porta_adsl: Integer;
   cmd_slot, prompt_slot, cmd_porta, prompt_porta: String;
   slotAD, portaAD, ID_Porta: String;
...
// Script´s MA5103
        ...
        begin
          ...
          begin
                ...
                ProfileB1000(Profile,Up,Dn);                         // Analisa Vel Up-Down
...
if (success <> 1) then
  ...
  prompt_anterior := prompt;      // Memoriza ultimo comando para re-escrever comunicação com Dslam
...
begin
 ...
 //  recebeu := copy(resposta,0,500);

//   MemoResposta.Lines.Add('------- RECEBEU[ '+IntToStr(resposta_num) + ' ]--------');
//   MemoResposta.Lines.Add(recebeu);
//   MemoResposta.Lines.Add('----------------------------------------------');
   ...
   begin
        ...
        if ( pos('Down', resposta)>0 )and( pos('Unlock', resposta)>0 ) then PTstatus := 'Não sincroni';
        if ( pos('Down', resposta)>0 )and( pos('Lock', resposta)>0 ) then PTstatus := 'Bloqueada';

        if pos('Up', resposta)>0 then PTstatus := 'Sincronizada';
        // Não esta pegando Up, então...
        if (PTstatus <> 'Não sincroni')and(PTstatus <> 'Bloqueada') then PTstatus := 'Sincronizada';
   end;
...
procedure TMain.ProfileB820(var Profile, Up, Dn: String);
begin
   if pos('1536_320', UpperCase(resposta))>0 then begin Profile := '1536_320'; Dn := '1536'; Up :='320'; end;
   if pos('160_64', UpperCase(resposta))>0 then begin Profile := '160_64'; Dn := '160';  Up :='64'; end;
   if pos('1024_320', UpperCase(resposta))>0 then begin Profile := '1024_320'; Dn := '1024'; Up :='320'; end;
   if pos('608_160', UpperCase(resposta))>0 then begin Profile := '608_160'; Dn := '608'; Up :='160'; end;
   if pos('256_128', UpperCase(resposta))>0 then begin Profile := '256_128'; Dn := '256'; Up :='128'; end;
   if pos('320_160', UpperCase(resposta))>0 then begin Profile := '320_160'; Dn := '320'; Up :='160'; end;
   if pos('2048_416', UpperCase(resposta))>0 then begin Profile := '2048_416'; Dn := '2048'; Up :='416'; end;
   if pos('1024_512', UpperCase(resposta))>0 then begin Profile := '1024_512'; Dn := '1024'; Up :='512'; end;
   if pos('416_224', UpperCase(resposta))>0 then begin Profile := '416_224'; Dn := '416'; Up :='224'; end;   
   if pos('800_320', UpperCase(resposta))>0 then begin Profile := '800_320'; Dn := '800'; Up :='320'; end;
   if pos('608_320', UpperCase(resposta))>0 then begin Profile := '608_320'; Dn := '608'; Up :='320'; end;

   if MenuRastrear2.Checked then Rastrear('Profile5100( ' + Profile +', ' + Up + ', ' + Dn + ')');

end;

procedure TMain.ProfileB1000(var Profile, Up, Dn: String);
begin
...
begin
   ...
   // data := Time;
    ...
    ZTbPtNec.Locate('obj_porta',Porta,[]);
    ...
    // Este código edita o registro que foi localizado acima
       ZTbPtNec.Edit;
       ZTbPtNec.FieldByName('egd_status').Value := Status;
       ZTbPtNec.FieldByName('egd_profile').Value := Profile;
       ZTbPtNec.FieldByName('egd_vel_up').Value := VelUP;
       ZTbPtNec.FieldByName('egd_vel_dn').Value := VelDN;
       ZTbPtNec.FieldByName('egd_analise').Value := Info_analise;
       ZTbPtNec.FieldByName('egd_acao').Value := 'Monitorada';
       ZTbPtNec.FieldByName('egd_obs').Value := analise_st +', ' + velUP_pt +', ' + velDN_pt;
       ZTbPtNec.FieldByName('egd_data').Value := DateToStr(date());
       ZTbPtNec.UpdateRecord;
       ZTbPtNec.next;
