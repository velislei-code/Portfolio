...
//Pega data do sistema
  ...
  private SimpleDateFormat Ano = new SimpleDateFormat("YYYY");  			// Formato: 05Jun	
	 ...
	 * O Cmd abaixo deveria mas n�o funcionou....
	 * 		private mtaView mtaFrmPrincipal;
	 *  	mtaFrmPrincipal.FixeLinAtual(0);
	 *  
	 * bTabAlterada criada para acessar info, a partir do FrmPrincipal e l� alterar, 
	 * pois N�o consegui alterar valor da LinAtual a partir do FrmOpc(daqui para l�)
	 */	
  ...
  public String DialogAbrir(String sExtencao){
	  ...
	  if(sExtencao == "mta"){		  
		  	...
		  	objFileC.setFileFilter(FiltrarExtencao);		// Filtrar exten��o
	 ...
	 // objLog.Metodo("Arquivos().DialogAbrir(): " + objFileC.getFileView().toString());
	  ...
	  if(sExtencao == "mta"){ 
		...
		//  objFileC.setName("Proj.mta");
		  ...
		  if (iValorRet == JFileChooser.CANCEL_OPTION) {
			  NomeArq.setText("#Null");	// Se Cancelou, fixa valor de NomeArq como: "cancelado", o # � usado para testar cancelamento
			  Diretorio.setText("");
		  }
		  
		  // Formata diret�rio, sArquivo de retorno
  ...
  public void SalvarCsv(JTable jtTabela){
	  ...
	  FormatarCsv(jtTabela);	// Chama m�todo: Formata CSV, e Grava em Arq.txt
  }
  ...
  public boolean GravarTxt(String sArquivo, String sTexto){
	  ...
	  // Salva texto em fArquivo, no diret�rio passado como caminho
		// java.io.*;		
  ...
  public boolean GravarTxtSimples(String sNomeArq, String sTexto){
  	...
  	public String[] LerTxt(String sNomeArq){
		...
		int iL = 0;
	 	try { 
			...
			String sLin = lerArq.readLine(); // l� a primeira linha // a vari�vel "linha" recebe o valor "null" quando o processo // de repeti��o atingir o final do arquivo texto
			while (sLin != null) { 
				vLinha[iL] = lerArq.readLine(); // l� da segunda at� a �ltima linha
	...
	public void FormatarCsv(JTable jtTabela){
		...
		CxDialogo objCxD = new CxDialogo();
			...
			String sArquivo = DialogSalvar("csv", null);			// Chama Dialog-salvar, pega diret�rio 
			
			// Testa retorno de diret�rio: (# = Sinal de cancelar)
			if( sArquivo.indexOf('#') < 0){			
				// Testa a exist�ncia de exten��o	  
				if(sArquivo.indexOf('.') > 0)	sArqExt = sArquivo;		// Sem inser��o de exten��o
				else	sArqExt = sArquivo + "." + sExtencao;			// Acrescenta exten��o do fArquivo
			 
				int iNumCol = jtTabela.getColumnCount();	// Pega num colunas Tab
				int iNumLin = jtTabela.getRowCount();		// Pega num Linhas Tab
				String sLinhaCSV = objDef.sTabTitulo;		// Pega t�tulos da tabela 
				GravarTxt(sArqExt, sLinhaCSV);				// Salva linha(T�tulos) no fArquivo	
				...
				objCxD.Aviso("A��o cancelada !", objDef.bMsgCancel);
			}
	}
	...
	public void SalvarMtaIni(JTable jTabIni, String sNomeArq, int iTotLin) throws IOException{
		// Salva dados da Tabela em arquivo *.ini
			...
			try {  
				...
				// sNomeArq entra como nome padrao PrjTeste, mas sDirArq pega digita��o do usu�rio
				String sDirArq = DialogSalvar("mta", sNomeArq);		// Chama SaveDialog, pega diret�rio				
				
				// Ver.se nome possui exten��o, caso n�o...insere
				...
				if( sDirArq.indexOf('#') < 0){
					...
					if (!fArquivo.exists()) {	// Se o arquivo n�o existe...				
	...
	public void LerMtaIni(JTable jtTabela, String sDirArq) throws IOException{
		...
		//try{
		 ...
		 // Memoriza Planilha Atual(P/ comparar altera��es - nova carga de arquivo)
		//String sMemPlanAtual = sDirArq;	
		String chave ="*.mta";
		
		if(sDirArq == null){
			sDirArq = DialogAbrir("mta");		// Chama DialogAbrir, pega diret�rio	
		}
		...
		if( sDirArq.indexOf('#') < 0){
			 ...
			 *  verifica se novo arquivo N�O CONTEM parte do nome(ultimos 10 char) do antigo arquivo
			 */
	    	...
	    	if(bCarregarTab){
	    		...
	    		// L� total de linhas no arquivo(iL = XX)
	    	...
	    	// Deveria atualizar titulo do Form mtaView mas n�o funciona
	    //	FrmPrincipal.setTitle(sDirArq);
	    	
	    	//mtaView.getFrames().getClass().getSuperclass().getTypeParameters().sTeste = sDirArq;
	    	
		}else{	objCxD.Aviso("A��o cancelada !", objDef.bMsgCancel);
		//	return null;
		}
...
public void LerMtaIni_bak19nov(JTable jtTabela, String sDirArq) throws IOException{
	...
	public void SalvarConfig() throws IOException{
		// Salva dados da Tabela em arquivo *.ini
			...
			try { 
					...
					String sDirArq = objDef.DirRoot + "config.ini";		// Chama SaveDialog, pega diret�rio				
	...
	// Salva dados da Tabela em arquivo *.ini
		...
		/*
		 * Devido a diferen�a de objetos(Defini��es) 
		 * os valore abaixo n�o s�o os mesmos, do objeto criado no mtaView
		 * ent�o, qdo uma var.� atualizada no objeto do mtaView(objDef.xxx), este n�o carrega 
		...
		// Salva dados da Tabela em arquivo *.ini
			...
			try {  
				...
				if( sDirArq.indexOf('#') < 0){
					...
					objLog.Metodo("mtaView().AutoBackup(A��o cancelada !)");
				}
	...
	public void criarDiretorio() {
		/*
		 * Testa se diretorios de mta n�o existem...
		 * Cria:
        ...
        try {
        	 ...
        	 File diretorio = new File(objDef.DirMtaDoc); // ajfilho � uma pasta!
        	 File dirBak = new File(objDef.DirBak); // ajfilho � uma pasta!
             if (!diretorio.exists()) {
                diretorio.mkdirs(); //mkdir() cria somente um diret�rio, mkdirs() cria diret�rios e subdiret�rios.
                dirBak.mkdirs();
             }
             objLog.Metodo("Ferramentas().criarDiretorio(Diret�rios criados: "+ dirBak + ")");
        } catch (Exception ex) {
            //JOptionPane.showMessageDialog(null, "Erro ao criar o diretorio");            
        	objLog.Metodo("Ferramentas().criarDiretorio(Diret�rio j� existente, "+ ex + ")");
        }
	...
	public void LerLicenca() throws IOException{
		/*
		 * O Ctrl da Licen�a Demo sera feito em 3 arquivos .ini
		 * C:/tmp/setup.ini
		 ...
		 * 		"Uservld": 	Data de validade da licen�a (2018)					
		 */
			...
			String sVarIniAnoPC = "Uservld";					// Var-ini data ano limite para licen�a

			/*
			 * Dados de controle
			 * Os anos s�o os utilizados, mas adicionei mais alguns caracteres
		...
		// Primeira consulta: mtaView/config.ini 
			...
			// L� arquivo.ini -> Anos(encryptografados)
	...
	public void SalvarLicenca() throws IOException{
		 ...
		 */
			...
			try { 
				...
				ArqIni.set(sChaveLcc, sVarIniAnoPC, sAnoPcCript );		// Ultimo ano v�lido(efeito catraca, registra para evitar retornos de data pelo user)
			...
			try { 
					 ...
					 * s� pra dar uma forma mais consistente	
					 */
