 ...
 *   Sistemas de controle a esteira de caminhada
...
long memRpm;           // Armazena m�dia de 100 leituras de tens�o
long f_Rpm;            // Armazena tens�o em Volts
int8 bMemPulso;        // Memoriza estado do pulso para comparar e detectar borda descida
int16 iContaPulso = 0; // Contagem de pulsos de entrada p/ Rpm
int16 iContaCiclo = 0; // Contagem de ciclos de varredura p/ Rpm
float iFrequencia = 0;
int16 iSeg = 0; // Contagem de Tempo em segundos
int16 iMin = 0; // Contagem de Tempo em minutos
...
int16 RPM();
long medias_Rpm();
long lerPortaAdc(int8 canal);
int8 PulsoDetecte(int8 bPulso); // Check se Pulso � borada de descida
float ContaRpm(Boolean bPulso);

void main()
{

   int16 iMiliSeg = 0; // Ctrl tempo em milisegundos, desconto de delay(50ms) no while

   set_tris_a(0x0F); // Seta Pinos A4 a A7 sa�da(0000) e A0 a A3 como entrada(1111)(sendo A0eA1 anal�gica) - ou...set_tris_a(0b00001111);
                     // Pinos A0 e A1 s�o entradas do potenciometro
                     // Pinos A2eA3 s�o entradas dos bot�es(Up e Down)

   set_tris_b(0x00); // LCD - e PWM, Seta pinos B como sa�da[0x0(0000)0(0000)] ou set_tris_b(0b00000000);

   // ADCON0 = 0X0F;       // Config os pinos do PortA( como digitais
   //  INTCON2 = 0x7F;   // Habilita os Pull-Ups internos do PortB(0111 1111b)

   // Seta Pino a5 p/ 1 conectado ao Pino MCLR, com isso p�e o MC em modo funcionamento
   // output_high(PIN_A5);
   ...
   while (TRUE)
   {
      ...
      // A cada 20 miliseg(1000ms50ms(delays)) incrementa segundos
      if (iMiliSeg >= 20)
      {
         iMiliSeg = 0;
         iSeg++;
      ...
      int16 tensao = Volts();
      // int16 iRpm = RPM();
      float fContaRpm = ContaRpm(input(pin_a4));

      // PWM-0 - N�o esta em uso
      // set_adc_channel(0);                   // Select channel AN0 - RPM
      ...
      duty1 = lerPortaAdc(1); // Read from AN0 and store in i - AN0 varia de 0 a 1023
      ...
      printf(lcd_putc, "\f %lu Vcc - %lu %lu %lu \n\r %lu(Duty)", tensao, iMin, iSeg, iMiliSeg, duty1); // L� a entrada da porta analogica(Volt()) e imprimir no lcd (\f limpa o lcd)
      delay_ms(50);

      iMiliSeg++; // Incementa ctrl de tempo em Milisegundos - considerar atraso de 50ms do delay(40+10) acima
   }
}

long lerPortaAdc(int8 canal)
{
   // faz leitura da porta analogica, retorna valor na porta
   set_adc_channel(canal);
   delay_us(20);
   return read_adc();
}

int8 PulsoDetecte(int8 bPulso)
{
   // Check se pulso atual � zero, Compara pulso anterior com pulso atual e ve se mudou de 1 p/ 0(Borda de descida
   if (bPulso == 0)
   { // Se agora � falso(Zero)
      if (bMemPulso == 1)
      {                      // Se Pulso anterior(Memorizado) == 1 e agora � zero... borda de descida
         bMemPulso = bPulso; // Memoriza estado atual
         return 0;
      }
      else
      {
...
float ContaRpm(int8 bPulso)
{
   // calcula, conta frequencia de pulsos

   if (PulsoDetecte(bPulso) == 0)
   { // Borda de descida
      ...
      if (iContaCiclo >= 60)
      {
         iFrequencia = iContaPulso / 60;
         iContaPulso = 0;
         iContaCiclo = 0;
      }

      // iFrequencia = iContaCiclo;
   }
...
{                          // Fun��o que calcula a m�dia de 100 leituras de tens�o
   ...
   { // Soma as 100 leituras
      ...
      volt_armazena += lerPortaAdc(1); // leitura da entrada analogica
   }
...
int16 RPM()
{
   ...
   // memRPM = medias_RPM();          // Recebe o valor m�dio da tens�o retornando pela fun��o average_volt
   // memRPM = read_adc();                    // leitura da entrada analogica
   memRPM = lerPortaAdc(0); // Read from AN0 and store in i - AN0 varia de 0 a 1023
   int16 f_Rpm = memRPM;    //(memRPM*48)/1023;      // 400 = 40.0 Volt(tens�o m�x, com 1 decimal) - Converte valor para Volts(1023 Resolu��o de 10 bits do conversor AD)
...
{                         // Fun��o que calcula a m�dia de 100 leituras de tens�o
                          // garante maior precis�o na medida, se houver alma leitura erronea por ru�dos, ele vai corrigir pois retorna a m�dia(100) das leituras
   unsigned char i;       // Var de itera��es
   long rpm_armazena = 0; // var local p/ armazenar o valor de tens�o

   for (i = 0; i < 100; i++)
   { // Soma as 100 leituras
      // volt_armazena += ADC_Read(0);    // Temp_store = temp_store + ADC_Read(0) (Faz soma das 100 leitiras)
      rpm_armazena += read_adc(); // leitura da entrada analogica
   }

   return (rpm_armazena / 100); // Retorna a media das itera��es
